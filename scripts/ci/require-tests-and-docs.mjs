...
const defaultBaseRef = (() => {
  try {
    ...
    throw new Error("Unable to resolve base ref: neither origin/main nor origin/master exists.");
  }
})();
...
const isProdCode = (file) =>
  /^(src|app|server|api)\//.test(file) && !/\.(test|spec)\.[cm]?[jt]sx?$/.test(file);
...
const isArchChange = (file) =>
  /^src\/(auth|db|infra|queue|events|architecture)\//.test(file) || /^infra\//.test(file);
...
if (apiChanged && !docsChanged)
  failures.push("Policy failure: API/command changes without docs/OpenAPI updates.");
if (archChanged && !adrChanged)
  failures.push("Policy failure: architecture-impacting change without ADR.");
