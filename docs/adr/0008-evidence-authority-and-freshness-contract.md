# ADR 0008: Evidence Authority and Freshness Contract
...
- Older `RUN_MANIFEST_V1` bundles remain diagnostically inspectable, but the current
  authority-contract check returns a deterministic blocker result. They are non-authorizing,
  receive no synthesized provenance, and require an explicitly versioned migration to become V2.
