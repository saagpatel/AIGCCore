...
fn malformed_provenance_cross_case_and_cache_reuse_remain_unknown() {
    ...
    assert_eq!(
        base_authority.evaluate_claim(CLAIM_LOCAL_CONTROLLED_EXECUTION, &cross_case),
        EvidenceClaimDecision::Unknown
    );

    let mut contaminated = base_authority.clone();
    ...
    for unknown_revision in ["unknown", " UNKNOWN "] {
        let mut unknown = authority();
        unknown.source.source_revision = unknown_revision.to_string();
        let mut unknown_context = context();
        unknown_context.expected_source_revision = unknown_revision.to_string();
        assert!(unknown.validate_internal("case-a", AUDIT).is_err());
        assert_eq!(
            unknown.evaluate_claim(CLAIM_LOCAL_CONTROLLED_EXECUTION, &unknown_context),
            EvidenceClaimDecision::Unknown
        );
    }
}
