...
#[test]
fn legacy_run_manifest_v1_is_inspectable_but_non_authorizing() {
    let temp = tempfile::tempdir().unwrap();
    let bundle_root = temp.path().join("legacy_bundle");
    let bundle_zip = temp.path().join("legacy_bundle.zip");
    let mut inputs = make_inputs(&bundle_root).unwrap();
    inputs.bundle_info.schema_versions.run_manifest = "RUN_MANIFEST_V1".to_string();

    EvidenceBundleBuilder::build_dir(&bundle_root, &inputs).unwrap();
    remove_evidence_authority(&bundle_root);
    EvidenceBundleBuilder::build_zip(&bundle_root, &bundle_zip).unwrap();

    let summary = BundleValidator::new_v3()
        .validate_zip(&bundle_zip, PolicyMode::STRICT)
        .unwrap();
    let authority_check = summary
        .checks
        .iter()
        .find(|check| check.check_id == "CHK.EVIDENCE.AUTHORITY_CONTRACT")
        .expect("authority check must remain visible for a legacy bundle");

    assert_eq!(summary.overall, "FAIL");
    assert_eq!(authority_check.severity, "BLOCKER");
    assert_eq!(authority_check.result, "FAIL");
    assert_eq!(
        authority_check.message,
        "legacy RUN_MANIFEST_V1 is inspectable but non-authorizing; migrate to RUN_MANIFEST_V2 with explicit evidence authority"
    );
    assert!(summary.checks.iter().any(|check| {
        check.check_id == "CHK.BUNDLE.REQUIRED_FILES" && check.result == "PASS"
    }));
}

#[test]
fn run_manifest_v2_without_authority_fails_closed() {
    let temp = tempfile::tempdir().unwrap();
    let bundle_root = temp.path().join("missing_authority_bundle");
    let bundle_zip = temp.path().join("missing_authority_bundle.zip");
    let inputs = make_inputs(&bundle_root).unwrap();
    ...
    let authority_check = summary
        ...
        .iter()
        .find(|check| check.check_id == "CHK.EVIDENCE.AUTHORITY_CONTRACT")
        .expect("authority check must fail closed");

    assert_eq!(summary.overall, "FAIL");
    assert_eq!(authority_check.result, "FAIL");
    assert!(authority_check
        .message
        .contains("failed to read run_manifest.json"));
    assert!(authority_check.message.contains("evidence_authority"));
}

fn remove_evidence_authority(bundle_root: &Path) {
    let manifest_path = bundle_root.join("run_manifest.json");
    let mut manifest: serde_json::Value =
        serde_json::from_slice(&fs::read(&manifest_path).unwrap()).unwrap();
    let removed = manifest
        .as_object_mut()
        .expect("run manifest must be an object")
        .remove("evidence_authority");
    assert!(removed.is_some(), "fixture must begin with bound authority");
